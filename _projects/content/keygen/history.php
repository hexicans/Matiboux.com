...
if(!$_Oli->isEmptyPostVars()) {
	if(empty($_Oli->getPostVars('decryptPassword')))
		...
		$_Oli->setCookie('decryptPassword', serialize(array('encryptedPassword' => $encryptedPassword, 'encryptPasswordDate' => $_Oli->getLastInfoMySQL('keygen_encrypt_passwords', 'creation_date'))), $keepDelay, '/', '.' . $_Oli->getSetting('domain'));
...
<title>Historique - <?php echo $_Oli->getSetting('name'); ?></title>
...
<?php if($_Oli->getUrlParam(2) == 'delete' AND !empty($_Oli->getUrlParam(3)) AND $resultCode == 'CONFIRMATION_NEEDED') { ?>
	<div class="message message-warning">
		<div class="container">
			...
			<p>
				<a href="<?php echo $_Oli->getUrlParam(0) . $_Oli->getUrlParam(1); ?>/<?php echo $_Oli->getUrlParam(2); ?>/<?php echo $_Oli->getUrlParam(3); ?>/confirmed" class="btn btn-primary btn-block">
					<i class="fa fa-check fa-fw"></i> J'autorise la suppression définive de ces keygens
				</a>
				<a href="<?php echo $_Oli->getUrlParam(0) . $_Oli->getUrlParam(1); ?>/" class="btn btn-danger btn-block">
		...
		<?php if(!empty($yourKeygens)) { ?>
			<?php if(!empty($decryptPassword)) { ?>
				<?php if($_Oli->getUrlParam(2) == 'edit' AND !empty($_Oli->getUrlParam(3)) AND $_Oli->isExistInfosMySQL('keygen_history', array('id' => $_Oli->getUrlParam(3))) AND !$updatedKeyGen) { ?>
					<a href="<?php echo $_Oli->getUrlParam(0); ?>history/" class="btn btn-primary btn-xs">
					...
					<table class="table table-hover">
						...
						<tbody>
							...
							<?php foreach($yourKeygens as $eachKeygen) { ?>
								...
								<tr id="<?php echo $eachKeygen['id']; ?>">
									...
									<td>
										<a href="<?php echo $_Oli->getUrlParam(0) . $_Oli->getUrlParam(1); ?>/edit/<?php echo $eachKeygen['id']; ?>" class="btn btn-primary btn-xs">
											Edit <i class="fa fa-pencil fa-fw"></i>
										</a>
									</td>
									<td>
										<a href="<?php echo $_Oli->getUrlParam(0) . $_Oli->getUrlParam(1); ?>/delete/<?php echo $eachKeygen['id']; ?>" class="btn btn-danger btn-xs">
						...
						<tfoot>
							<tr>
								...
								<td>
									<a href="<?php echo $_Oli->getUrlParam(0); ?>history/delete/" class="deleteSelected btn btn-danger btn-xs">
