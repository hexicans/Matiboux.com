...
/** ------------------------------ */
/**  Login Page for Oli Framework  */
/** ------------------------------ */

if(!$_Oli->getAccountsManagementStatus())
	header('Location: ' . $_Oli->getOption('url'));
else if($_Oli->getUrlParam(2) == 'logout'
AND !empty($_Oli->getAuthKey())) {
	$_Oli->logoutAccount();
	$resultCode = 'LOGOUT_OK';
}
else if($_Oli->getUrlParam(2) == 'change-password'
AND !empty($_Oli->getUrlParam(3))
AND $_Oli->isExistAccountInfos('REQUESTS', array('activate_key' => $_Oli->getUrlParam(3)))) {
	if(isset($_POST['newPassword'])) {
		...
		AND $_Oli->getAccountInfos('REQUESTS', 'action', array('activate_key' => $_Oli->getUrlParam(3))) == 'change-password') {
			if(strtotime($_Oli->getAccountInfos('REQUESTS', 'expire_date', array('activate_key' => $_Oli->getUrlParam(3)))) >= time()) {
				...
				$resultCode = 'CHANGE_PASSWORD_EXPIRED';
		}
		else
			$resultCode = 'CHANGE_PASSWORD_FAILED';
	}
}
else if($_Oli->verifyAuthKey()) {
	header('Location: ' . $_Oli->getOption('url'));
}
else if($_Oli->getUrlParam(2) == 'activate'
AND !empty($_Oli->getUrlParam(3))) {
	...
	AND $_Oli->getAccountInfos('REQUESTS', 'action', array('activate_key' => $_Oli->getUrlParam(3))) == 'activate') {
		if(strtotime($_Oli->getAccountInfos('REQUESTS', 'expire_date', array('activate_key' => $_Oli->getUrlParam(3)))) >= time()) {
			...
			$resultCode = 'ACTIVATE_EXPIRED';
	}
	else
		$resultCode = 'ACTIVATE_FAILED';
}
else if($_Oli->getUrlParam(2) == 'recover'
AND isset($_POST['email'])) {
	if(empty($_POST['email'])) {
		$resultCode = 'EMAIL_EMPTY';
	}
	else if($_Oli->isExistAccountInfos('REQUESTS', array('username' => $_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])), 'action' => 'change-password'))
	AND strtotime($_Oli->getAccountInfos('REQUESTS', 'expire_date', array('username' => $_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])), 'action' => 'change-password'))) >= time())
...
OR isset($_POST['password'])) {
	if(empty($_POST['username'])) {
		...
		else if($_Oli->registerAccount($_POST['username'], $_POST['password'], $_POST['email'])) {
			if($_Oli->getRegisterVerificationStatus())
				$resultCode = 'REGISTER_CHECK_MAIL';
			else
				$resultCode = 'REGISTER_OK';
		}
		else
			$resultCode = 'REGISTER_FAILED';
	}
	else {
		if($_Oli->verifyLogin($_POST['username'], $_POST['password'])) {
		...
		else if(!$_Oli->isExistAccountInfos('ACCOUNTS', array('username' => $_POST['username'])))
			$resultCode = 'UNKNOWN_USER';
		else if($_Oli->getUserRightLevel(array('username' => $_POST['username'])) == $_Oli->translateUserRight('NEW-USER'))
...
<title>Login - <?php echo $_Oli->getOption('name'); ?></title>

</head>
<body>

<div class="header">
	<h1><a href="<?php echo $_Oli->getOption('url'); ?>"><?php echo $_Oli->getOption('name'); ?></a></h1>
...
<?php } else if($resultCode == 'CHANGE_PASSWORD_EXPIRED') { ?>
	<div class="message message-error">
		<div class="content">
			<h2>Le changement de mot de passe n'a pas aboutie, le code d'activation a déjà expiré</h2>
		</div>
	</div>
...
<div class="module form-module">
	<?php if($_Oli->getUrlParam(2) == 'recover' AND !$hideRecoverUI) { ?>
		<div class="form">
			...
			<form action="<?php echo $_Oli->getOption('url'); ?>login.php/recover" method="post">
				<input type="email" name="email" placeholder="Email Address" />
				<button type="submit">Recover</button>
			</form>
		</div>
		<div class="cta"><a href="<?php echo $_Oli->getOption('url'); ?>login.php">Login to your account</a></div>
	<?php } else if($_Oli->getUrlParam(2) == 'change-password' AND $_Oli->isExistAccountInfos('REQUESTS', array('activate_key' => $_Oli->getUrlParam(3))) AND !$hideChangePasswordUI) { ?>
		<div class="form">
			...
			<form action="<?php echo $_Oli->getOption('url'); ?>login.php/change-password/<?php echo $_Oli->getUrlParam(3); ?>" method="post">
	...
	<?php } else { ?>
		...
		<div class="form">
			...
			<form action="<?php echo $_Oli->getOption('url'); ?>login.php" method="post">
		...
		<div class="form">
			...
			<form action="<?php echo $_Oli->getOption('url'); ?>login.php/register" method="post">
				<input type="text" name="username" placeholder="Username" />
				<input type="password" name="password" placeholder="Password" />
				<input type="email" name="email" placeholder="Email Address" />
		...
		<div class="cta"><a href="<?php echo $_Oli->getOption('url'); ?>login.php/recover">Forgot your password?</a></div>
	<?php } ?>
</div>
