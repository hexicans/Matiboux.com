...
if($_Oli->getUrlParam(2) == 'logout' AND !empty($_Oli->getAuthKey())) {
...
else if($_Oli->getUrlParam(2) == 'change-password' AND !empty($_Oli->getUrlParam(3)) AND $_Oli->isExistAccountInfos('REQUESTS', array('activate_key' => $_Oli->getUrlParam(3)))) {
	if(isset($_POST['newPassword'])) {
		if(empty($_POST['newPassword'])) $resultCode = 'PASSWORD_EMPTY';
		...
		AND $_Oli->getAccountInfos('REQUESTS', 'action', array('activate_key' => $_Oli->getUrlParam(3))) == 'change-password') {
			if(strtotime($_Oli->getAccountInfos('REQUESTS', 'expire_date', array('activate_key' => $_Oli->getUrlParam(3)))) >= time()) {
				$_Oli->updateAccountInfos('ACCOUNTS', array('password' => $_Oli->hashPassword($_POST['newPassword'])), array('username' => $_Oli->getAccountInfos('REQUESTS', 'username', array('activate_key' => $_Oli->getUrlParam(3)))));
...
else if($_Oli->getUrlParam(2) == 'recover' AND isset($_POST['email'])) {
	if(empty($_POST['email'])) $resultCode = 'EMAIL_EMPTY';
	else if($_Oli->isExistAccountInfos('REQUESTS', array('username' => $_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])), 'action' => 'change-password')) AND strtotime($_Oli->getAccountInfos('REQUESTS', 'expire_date', array('username' => $_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])), 'action' => 'change-password'))) >= time())
		$resultCode = 'REQUEST_ALREADY_EXIST';
	else {
		$activateKey = $_Oli->createRequest($_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])), 'change-password');
		
		$email = $_POST['email'];
		$subject = 'Changez votre mot de passe';
		$message = 'Bonjour ' . $_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])) . ', <br />';
		...
		$message .= 'Vous avez jusqu\'au ' . date('d/m/Y', strtotime($_Oli->getAccountInfos('REQUESTS', 'expire_date', array('username' => $_Oli->getAccountInfos('ACCOUNTS', 'username', array('email' => $_POST['email'])), 'action' => 'change-password'))) + $_Oli->getRequestsExpireDelay()) . ', <br />';
	...
	if(empty($_POST['username'])) $resultCode = 'USERNAME_EMPTY';
	else if(empty($_POST['password'])) $resultCode = 'PASSWORD_EMPTY';
	else if(!$_Oli->isExistAccountInfos('ACCOUNTS', array('username' => $_POST['username']), false) AND isset($_POST['email'])) {
		if(empty($_POST['email'])) $resultCode = 'EMAIL_EMPTY';
		else if($_Oli->isProhibitedUsername($_POST['username'])) $resultCode = 'PROHIBITED_USERNAME';
		else if($_Oli->registerAccount(trim($_POST['username']), $_POST['password'], strtolower(trim($_POST['email'])))) {
		...
		if(!$_Oli->isExistAccountInfos('ACCOUNTS', array('username' => $_POST['username']), false)) $resultCode = 'UNKNOWN_USER';
		else if($_Oli->getUserRightLevel(array('username' => $_POST['username'])) == $_Oli->translateUserRight('NEW-USER')) $resultCode = 'NOT_ACTIVATED';
		else if($_Oli->getUserRightLevel(array('username' => $_POST['username'])) == $_Oli->translateUserRight('BANNED')) $resultCode = 'BANNED_USER';
		else if($_Oli->verifyLogin($_POST['username'], $_POST['password'])) {
			$loginDuration = ($_POST['rememberMe']) ? 15*24*3600 : 24*3600; // 15 days : 1 day
			if($_Oli->loginAccount($_POST['username'], $_POST['password'], $loginDuration)) {
				$resultCode = 'LOGIN_OK';
				
				if(!empty($_POST['referer'])) header('Location: ' . $_POST['referer']);
...
<div class="module form-module">
	<?php if($_Oli->getUrlParam(2) == 'recover' AND !$hideRecoverUI) { ?>
		<div class="form">
			...
			<form action="<?php echo $_Oli->getUrlParam(0); ?>login.php/recover" method="post">
				<input type="email" name="email" placeholder="Email Address" />
				<button type="submit">Recover</button>
			</form>
		</div>
		<div class="cta"><a href="<?php echo $_Oli->getUrlParam(0); ?>login.php">Login to your account</a></div>
	<?php } else if($_Oli->getUrlParam(2) == 'change-password' AND $_Oli->isExistAccountInfos('REQUESTS', array('activate_key' => $_Oli->getUrlParam(3))) AND !$hideChangePasswordUI) { ?>
		<div class="form">
			...
			<form action="<?php echo $_Oli->getUrlParam(0); ?>login.php/change-password/<?php echo $_Oli->getUrlParam(3); ?>" method="post">
	...
	<?php } else { ?>
		...
		<div class="form">
			...
			<form action="<?php echo $_Oli->getUrlParam(0); ?>login.php" method="post">
				<input type="text" name="username" placeholder="Username" />
				<input type="password" name="password" placeholder="Password" />
				<?php if(!empty($_POST['referer']) OR !empty($_SERVER['HTTP_REFERER'])) { ?>
					<input type="hidden" name="referer" value="<?php echo (!empty($_POST['referer'])) ? $_POST['referer'] : $_SERVER['HTTP_REFERER']; ?>" />
				<?php } ?>
		...
		<div class="form">
			...
			<form action="<?php echo $_Oli->getUrlParam(0); ?>login.php/register" method="post">
				<input type="text" name="username" placeholder="Username" />
				<input type="password" name="password" placeholder="Password" />
				<input type="email" name="email" placeholder="Email Address" />
		...
		<div class="cta"><a href="<?php echo $_Oli->getUrlParam(0); ?>login.php/recover">Forgot your password?</a></div>
	<?php } ?>
</div>
